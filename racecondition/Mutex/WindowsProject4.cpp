// WindowsProject4.cpp: 응용 프로그램의 진입점을 정의합니다.
//
                     ...
                     _In_ int       nCmdShow)
{
	...
	execinfo.lpFile = _T("SejongATM.exe");
	execinfo.lpParameters = _T("");
	execinfo.fMask = SEE_MASK_FLAG_NO_UI | SEE_MASK_NOCLOSEPROCESS;
	execinfo.nShow = SW_SHOWDEFAULT;

	HANDLE hMutexInstance;
	hMutexInstance = CreateMutex(NULL, FALSE, _T("Unique Name of Mutex"));
	if (GetLastError() == ERROR_ALREADY_EXISTS)
	{
		MessageBox(NULL, TEXT("Someone is using the system. "), TEXT("Execution failed"), MB_OK);

	}
	else {

		WaitForSingleObject(hMutexInstance, INFINITE);
		ShellExecuteEx(&execinfo);
		WaitForSingleObject(execinfo.hProcess, 300000);
		MessageBox(NULL, TEXT("It has been 5minutes. Good bye!"), TEXT(""), MB_OK);
